...
export default function SwipeUp(props: ExampleProps) {
    ...
    return (
        <GestureHandlerRootView
            className={`absolute w-screen h-screen bottom-0 left-0`}
        >
            <>
                {openContent && (
                    <Pressable
                        className={`z-40 absolute transition-all ease-in-out bottom-0 left-0 w-full items-center justify-center h-screen bg-[#00000060]`}
                        onPress={onClose}
                    />
                )}
                <GestureDetector gesture={pan}>
                    <Animated.View
                        ...
                        className={`absolute bottom-0 z-50 h-full w-full px-2 pt-4 pb-1 ease-in-out rounded-t-3xl cursor-grab items-center justify-start ${
